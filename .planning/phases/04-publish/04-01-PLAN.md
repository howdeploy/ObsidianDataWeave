...
must_haves:
  truths:
    ...
    - "No personal vault paths or secrets exist in any tracked non-planning file (scripts, configs, templates)"
  artifacts:
    - path: "install.sh"
...
Verify LICENSE exists and contains "MIT". Verify .gitignore contains all new patterns: `grep -c "atom-plan.json" .gitignore` returns 1. Run `git ls-files` and confirm config.toml and processed.json are NOT in the tracked list.
  ...
  <done>MIT LICENSE file exists. .gitignore covers virtual environments, editor configs, staging artifacts, and logs. No personal paths exist in any tracked non-planning file (scripts, configs, templates).</done>
</task>
...
- No personal paths or secrets exist in tracked non-planning files (scripts, configs, templates)
</success_criteria>
